...
db.once('open', ()=> {
	...
	*/
	...
	//删除数据
	/*
	UserModel.deleteOne({name:"Jane"},(err,result)=>{
		if(err){
			console.log("deleteOne data err::",err)
		}else{
			console.log(result);
		}
	})
	*/
	UserModel.deleteMany({age:{$gt:49}},(err,result)=>{
		if(err){
			console.log("deleteMany data err::",err)
		}else{
			console.log(result);
		}
	})
